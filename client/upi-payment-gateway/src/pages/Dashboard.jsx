...
function Dashboard() {
        ...
        loginPollRef.current = setInterval(async () => {
            try {
                ...
                if (data.loggedIn) {
                    setPaytmStep('connected');
                    stopLoginPolling();
                    showToast('🎉 Paytm connected! Auto-verification active.');
                    fetchPaytmStatus();
                }
            } catch { }
        }, 3000);
        ...
        try {
            ...
            if (data.success) {
                showToast(data.matches > 0
                    ? `✅ Auto-confirmed ${data.matches} payment(s)!`
    ...
    const handleConfirm = async (orderId) => {
        try {
            await fetch(`/api/update-status/${orderId}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ status: 'confirmed' }),
            });
            setTransactions(prev => prev.map(t =>
                t.orderId === orderId ? { ...t, status: 'confirmed' } : t
            ));
            showToast('✅ Confirmed!');
    ...
    const handleReject = async (orderId) => {
        try {
            ...
            setTransactions(prev => prev.map(t =>
                t.orderId === orderId ? { ...t, status: 'rejected' } : t
            ));
            showToast('❌ Rejected');
        } catch { showToast('Failed'); }
    };
    ...
    const getStatusBadge = (status, confirmedBy) => {
        const map = {
            ...
            confirmed: { cls: 'confirmed', label: confirmedBy === 'paytm-auto' ? '🤖 Auto-Confirmed' : '✅ Confirmed' },
                    ...
                    <div className="glass-card" style={{
                        ...
                        <div className="section-header">
                            <h3 className="section-title">
                                {isConnected ? '🟢' : '🔴'} Paytm Auto-Verification
                            </h3>
                            {isConnected && (
                                <span style={{
                        ...
                        {paytmStep === 'idle' && (
                            <div>
                                <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: 16 }}>
                                    Link your Paytm to auto-verify payments. Scan a QR code with your Paytm app to connect.
                                </p>
                                <button className="btn-primary" onClick={handleStartQRLogin}
                                    disabled={paytmLoading} style={{ maxWidth: 300 }}>
                                    {paytmLoading ? '⏳ Loading...' : '🔗 Link Paytm Account'}
                                </button>
                            </div>
                        )}

                        {/* LOADING — Puppeteer is opening Paytm */}
                        {paytmStep === 'loading' && (
                            <div style={{ textAlign: 'center', padding: 30 }}>
                                ...
                                <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>This takes ~10 seconds, please wait</p>
                            </div>
                        )}

                        {/* QR CODE — show QR for user to scan */}
                        ...
                        {paytmStep === 'connected' && (
                            <div>
                                <div style={{
                                    ...
                                    <div>
                                        ...
                                        <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginTop: 2 }}>
                                            Auto-checking passbook every 10s
                                        </p>
                                    </div>
                                ...
                                <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem' }}>
                                    💡 When someone pays, the system auto-matches credits from your Paytm passbook and confirms orders instantly.
                                </p>
                            </div>
                        )}
                    </div>
                                            ...
                                            <tr key={txn.orderId}>
                                                ...
                                                <td>
                                                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                                                        {txn.status === 'pending' && (
                                                            <>
                                                                <button className="btn-success" onClick={() => handleConfirm(txn.orderId)}
                                                                    style={{ padding: '4px 12px', fontSize: '0.75rem' }}>✅</button>
                                                                <button className="btn-danger" onClick={() => handleReject(txn.orderId)}
